...
pub fn generate_commands_file(
    ...
    let mut tauri_imports = vec!["invoke"];
    if commands_use_channels(commands) {
        tauri_imports.push("Channel");
    }
...
///
/// `upload(on_progress: Channel<P>, on_done: Channel<D>)`:
///   → `("UploadOnProgressChannelType", "P")`, `("UploadOnDoneChannelType", "D")`
///
/// The arg name is always part of the alias so names stay stable when a
/// command gains a second channel.
pub fn collect_channel_type_aliases(
    ...
    for cmd in commands {
        let pascal_cmd = crate::utils::to_pascal_case(&cmd.name);
        for arg in &cmd.args {
            if let RustType::Channel(inner) = &arg.ty {
                let pascal_arg = crate::utils::to_pascal_case(&arg.name);
                aliases.push((
                    format!("{}{}ChannelType", pascal_cmd, pascal_arg),
