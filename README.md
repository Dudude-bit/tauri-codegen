...
- **Tauri Integration**:
    ...
    - Supports `tauri::ipc::Channel<T>` command arguments: typed as `Channel<T>` in the generated signature (with the `Channel` import added when needed), plus an exported `<Command><Arg>ChannelType` alias for each channel payload in the types file.
